<?php 
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($reg = mysqli_fetch_assoc($result)) {
            ...
            $pdf->Cell(25,20, utf8_decode($reg['pCod']),0);
            $pdf->SetFont('Times','',9);
            $pdf->Cell(45,20, utf8_decode($reg['pNom']),0);
            $pdf->Cell(35,20, utf8_decode($reg['pTip']),0);
            $pdf->Cell(28,20, '     '.($reg['pCant']).'     unid',0);
            $pdf->Cell(35,20, utf8_decode($reg['pClas']),0);
            $pdf->Cell(30,20, utf8_decode($reg['pMarc']),0);
            $pdf->Cell(30,20, utf8_decode($reg['pSer']),0);
